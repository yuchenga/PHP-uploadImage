...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        //加水印
        if ($this->watermark && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            $waterFileName = $file['savepath'].$file['savename'].$this->waterPrefix;      //加完水印后的图片名
            $image =  getimagesize($filename);
            if(false !== $image) {
                import($this->imageClassPath);
                Image::water($filename,$this->watermark,$waterFileName);
            }
        }
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    if($this->watermark) {
                        if(1 == $this->thumbType){
                            Image::thumb2($waterFileName,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);
                        }else{
                            Image::thumb($waterFileName,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);                        
                        }
                    }else {
                        if(1 == $this->thumbType){
